...
const DoorwayDetailsModel: React.FC<CloseModelProps> = ({
  ...
  const initialState = useRef({
    ...
    addresses: [""],
    urls: [{ type: "", value: "https://example.com" }] as {
      type: string;
      value: string;
    }[],
  ...
  const [selectedDepartment, setSelectedDepartment] = useState<string | null>(
    initialState.current.selectedDepartment
  );
  const [firstName, setFirstName] = useState<string>(
    initialState.current.firstName
  );
  const [lastName, setLastName] = useState<string>(
    initialState.current.lastName
  );
  const [jobTitle, setJobTitle] = useState<string>(
    initialState.current.jobTitle
  );
  ...
  const [doorwayName, setDoorwayName] = useState<string>(
    initialState.current.doorwayName
  );
  ...
  const [emails, setEmails] = useState<{ value: string; type: string }[]>(
    initialState.current.emails
  );
  const [phoneNumbers, setPhoneNumbers] = useState<
    { value: string; type: string }[]
  >(initialState.current.phoneNumbers);
  const [addresses, setAddresses] = useState<string[]>(
    initialState.current.addresses
  );
  const [urls, setUrls] = useState<{ type: string; value: string }[]>(
    initialState.current.urls
  );
  const [selectedOption, setSelectedOption] = useState<string | undefined>(
    initialState.current.selectedOption
  );
  const [customValue, setCustomValue] = useState<string>(
    initialState.current.customValue
  );
  const [dropdownStates, setDropdownStates] = useState<{
    [key: string]: boolean;
  }>(initialState.current.dropdownStates);
  ...
  useEffect(() => {
    const hasChanged =
      ...
      JSON.stringify(phoneNumbers) !==
        JSON.stringify(initialState.current.phoneNumbers) ||
      JSON.stringify(addresses) !==
        JSON.stringify(initialState.current.addresses) ||
      ...
      JSON.stringify(dropdownStates) !==
        JSON.stringify(initialState.current.dropdownStates);
  ...
  const handleSelectOption = (
    setState: React.Dispatch<
      React.SetStateAction<{ value: string; type: string }[]>
    >,
    label: string,
    index: number,
    option: string
    ...
    setState((prev) =>
      prev.map((item, i) => (i === index ? { ...item, type: option } : item))
    );
                ...
                <div className="flex items-center gap-[8px]">
                  <span
                    className={`font-medium ${
                      item.type ? "text-petite" : "text-gray-400"
                    }`}
                  >
                  ...
                  <div className="p-[8px] flex flex-col gap-[8px]">
                    {["Work", "Home", "Custom"].map((option) => (
                      <div
                        ...
                        onClick={() =>
                          handleSelectOption(setState, label, index, option)
                        }
                      >
                        <span className="text-petite font-medium">
                          {option}
                        </span>
                      </div>
                    ))}
                    ...
                    <div className="p-[8px] flex flex-col gap-[8px]">
                      ...
                      <div className="flex gap-[10px]">
                        <button
                          ...
                          onClick={() =>
                            handleSelectOption(setState, label, index, "")
                          }
                        >
                          Cancel
                        </button>
            ...
            <div className="flex-grow">
              ...
              >
                <input
                  ...
                  onChange={(e) =>
                    index > 0 &&
                    handleFieldChange(index, e.target.value, setState)
                  }
  ...
  const renderFields = (
    ...
    <div className="flex flex-col gap-[12px] items-start">
      ...
      {state.map(
        (
          { value, type },
          index // Extracting value and type
        ) => (
          <div key={index} className="w-full flex flex-col gap-[4px]">
            ...
            <input
              ...
              value={value}
              onChange={(e) =>
                !readOnly && handleFieldChange(index, e.target.value, setState)
              }
              placeholder={placeholder}
              readOnly={readOnly}
            />
            {errors?.[index] && (
              <div className="text-red-500 text-xs m-2">{errors[index]}</div>
            )}
          </div>
        )
      )}
        ...
        >
          <div className="flex flex-col h-full">
            ...
            <div className="overflow-y-scroll flex-grow">
              {extra === true && (
                <div className="p-[24px] bg-brand-50 flex gap-[24px] items-center">
                  {/* User Info */}
                  <div className="flex gap-[12px] items-center flex-grow">
                    ...
                    <div className="flex flex-col flex-grow">
                      <span className="text-petite font-bold text-brand-950">
                        {userCard.name}
                      </span>
                      <span className="text-tiny font-regular text-gray-950">
                        {userCard.email}
                      </span>
                      <span className="text-tiny font-regular text-gray-500">
                        {userCard.doorways} Doorways
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="text-regular font-regular">
                    <div className="inline-flex rounded-[6px] text-petite font-semibold justify-center gap-[6px] transition-all duration-500 ease-in-out px-[12px] py-[6px] text-brand-500 hover:bg-brand-100 focus:border-[2px] focus:border-brand-300 cursor-pointer">
              ...
              >
                ...
                <div className="flex flex-col gap-6">
                    ...
                    <div className="max-w-[390px]">
                      ...
                      <div className="relative">
                        ...
                        >
                          {/* Selected Department */}
                          <span>
                            {selectedDepartment || "Select a device type..."}
                          </span>
                          {/* Dropdown Icon */}
                  ...
                  <div className="flex flex-col gap-[24px]">
                    ...
                    {renderUrlFields("URLs", urls, setUrls, "Enter Url", "url")}
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="flex-grow text-gray-950 text-small font-medium flex justify-end gap-[16px]">
                ...
                <button
                  ...
                  className={`inline-flex rounded-[6px] text-petite font-semibold justify-center gap-[6px] transition-all duration-300 ease-in-out px-[16px] py-[8px] 
                                    ${
                                      isDisabled
                                        ? "bg-brand-100 cursor-default"
                                        : "bg-themeColor cursor-pointer text-black"
                                    }`}
                >
