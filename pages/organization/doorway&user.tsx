...
const DoorwayandUser: React.FC = () => {
    ...
    const handleCheckboxChange = (doorway: Doorway, isChecked: boolean) => {
        if (isChecked) {
            ...
            setSelectedList((prev) => prev.filter((item) => item.name !== doorway.name));
        }
    };
    ...
    const handleDoorwayFilterChange = (category: keyof typeof selectedFiltersDoorway, value: string) => {
    ...
    const isAllSelected = doorways.length > 0 && selectedList.length === doorways.length;
...
const handleSelectAllChange = (isChecked: boolean) => {
    ...
    const filteredDoorways = doorways.filter((doorway) => {
        // Filter by search term
        const searchMatch =
            doorway.name.toLowerCase().includes(searchTerm.toLowerCase()) 
    ...
    return (
        <div className="flex flex-col w-full">
            ...
            <div className="px-8 min-md:px-[112px] mt-6 mb-16">
                <div className="flex flex-col gap-[32px]">
                    ...
                    <div className="flex flex-col min-md:flex-row justify-between items-center min-md:border-b-[1px] border-gray-100 w-full">
                        ...
                        {selectedTab === "Doorways" && <div className="min-md:p-0 pt-[32px] flex gap-[24px] min-md:items-center w-full justify-center min-md:w-fit">
                            ...
                            <div className="hidden min-lg:flex gap-[24px]">
                                ...
                                >
                                    ...
                                    <span className="whitespace-nowrap">Download Full List</span>
                                </div>
                    ...
                    {selectedTab === "Doorways" && <div>
                        <div className="flex flex-col gap-[16px]">
                            <div className="min-md:flex items-center">
                                <div className="flex flex-col min-md:flex-row items-center gap-4">
                                    ...
                                    <div className="w-full min-md:w-[280px] flex items-center focus:border-themeColor  border rounded-md p-2 border-gray-300">
                                        <input type="text" placeholder="Search by name " className="flex-grow outline-none " value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                                    </div>

                                    {/* Filter Button */}
                                    <button className="flex items-center gap-2 px-4 py-2 rounded-md bg-brand-500 text-white hover:bg-brand-600" onClick={() => setShowFilterDoorway(!showFilterDoorway)}>
                                        <span>Filter ({selectedFiltersDoorway.designTemplate.length + selectedFiltersDoorway.downloadStatus.length})</span>
                                    </button>
                                ...
                                <div className="hidden min-lg:flex flex-grow justify-end gap-[24px]">
                                    <div className="flex items-center relative">
                                        ...
                                        >
                                            ...
                                            <span className="whitespace-nowrap">Update Design Template</span>
                                        </button>
                                    </div>
                                    ...
                                    <div className="flex items-center relative">
                                        ...
                                        >
                                            ...
                                            <span className="whitespace-nowrap">Download Selected List</span>
                                        </button>
                                    </div>
                                    ...
                                    >
                                        ...
                                        <span className="whitespace-nowrap">Resend Email Invite</span>
                                    </button>
                                </div>

                            </div>
                            {showFilterDoorway && <div className="hidden min-md:block w-full border border-gray-100 rounded-[6px] shadow-lg p-4 transition-height">
                                <div className="flex flex-col overflow-hidden">
                                    <div className="flex justify-between items-center mb-[24px]">
                                        <span className="text-small font-semibold text-gray-950">Doorway Filter</span>
                                        <span className="text-tiny font-regular text-gray-500">
                                            {selectedFiltersDoorway.downloadStatus.length + selectedFiltersDoorway.designTemplate.length} Filters Applied
                                        </span>
                                    </div>
                                    ...
                                    <div className="flex gap-[24px] mb-[24px] min-xl:flex-row flex-col">
                                        ...
                                        <div className="w-full border border-gray-100 rounded-[6px] p-[16px] flex flex-col gap-[12px]">
                                            <div className="flex justify-between items-center">
                                                <span className="text-petite font-regular text-gray-950">Download Status</span>
                                                <span className="text-tiny font-regular text-gray-500">{selectedFiltersDoorway.downloadStatus.length} Selected</span>
                                            </div>
                                            <div className="border-b border-gray-100"></div>
                                            {doorwayFilterOptions.downloadStatus.map((status) => (
                                                <label key={status} className="flex gap-[8px] items-center cursor-pointer">
                                                    <input
                                                        type="checkbox"
                                                        checked={selectedFiltersDoorway.downloadStatus.includes(status)}
                                                        onChange={() => handleDoorwayFilterChange("downloadStatus", status)}
                                                        className="w-[16px] h-[16px] rounded-[2px] border-gray-100 focus:ring-brand-200"
                                                    />
                                                    <span className="text-petite font-medium truncate max-w-[200px]">{status}</span>
                                                </label>
                                            ))}
                                        ...
                                        <div className="w-full border border-gray-100 rounded-[6px] p-[16px] flex flex-col gap-[12px]">
                                            <div className="flex justify-between items-center">
                                                <span className="text-petite font-regular text-gray-950">Design Template</span>
                                                <span className="text-tiny font-regular text-gray-500">{selectedFiltersDoorway.designTemplate.length} Selected</span>
                                            </div>
                                            <div className="border-b border-gray-100"></div>
                                            {doorwayFilterOptions.designTemplate.map((status) => (
                                                <label key={status} className="flex gap-[8px] items-center cursor-pointer">
                                                    <input
                                                        type="checkbox"
                                                        checked={selectedFiltersDoorway.designTemplate.includes(status)}
                                                        onChange={() => handleDoorwayFilterChange("designTemplate", status)}
                                                        className="w-[16px] h-[16px] rounded-[2px] border-gray-100 focus:ring-brand-200"
                                                    />
                                                    <span className="text-petite font-medium truncate max-w-[200px]">{status}</span>
                                                </label>
                                    ...
                                    <div className="flex justify-end gap-[24px]">
                                        <button className="text-gray-500 hover:text-brand-500" onClick={handleClearDoorwayFilters}>
                                            Clear
                                        </button>
                                        <button className="px-[12px] py-[6px] text-white bg-brand-500 hover:bg-brand-400 rounded-[6px]" onClick={() => setShowFilterDoorway(false)}>
                                            Apply
                                        </button>
                                    </div>
                                </div>
                            ...
                            {selectedTab === "Doorways" && <div className="overflow-y-auto overflow-x-auto">
                                <table className="w-full">
                                    <thead>
                                        <tr className="border-b-[1px] border-gray-100">
                                            ...
                                            <th className="py-[12px] text-left text-gray-700 text-petite font-medium pr-[10px] pl-[10px] cursor-pointer select-none">
                                                <div className="flex items-center">Doorway Name</div>
                                            </th>
                                            <th className="py-[12px] text-left text-gray-700 text-petite font-medium pr-[10px] pl-[10px] cursor-pointer select-none">
                                            ...
                                            <th className="py-[12px] text-left text-gray-700 text-petite font-medium pr-[10px] pl-[10px]">
                                                <div className="flex items-center">Quick Actions</div>
                                            </th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                            ...
                                            >
                                                <td className="pr-[10px] pl-[10px]">
                                                    <div className="pl-[12px]">
                                                        <input
                                                            type="checkbox"
                                                            checked={selectedList.some(item => item.id === doorway.id)}
                                                            onChange={(e) =>
                                                                handleCheckboxChange(doorway, e.target.checked)
                                                            }
                                                ...
                                                <td className="pr-[10px] pl-[10px]">
                                                    <span
                                                        className={`px-[8px] py-[4px] ${doorway.type === "Primary"
                                                ...
                                                <td className="pr-[10px] pl-[10px]">
                                                    <span className="text-petite font-regular text-gray-500 flex gap-[12px] truncate invisible group-hover:visible">
                                                        ...
                                                        <div
                                                            ...
                                                            aria-disabled="false"
                                                        >
                                                            <span className="whitespace-nowrap">Resend Doorway</span>
                                                        </div>
                                                    </span>
                                                </td>
                                            </tr>
                                        ...
                                        <div className="flex flex-col gap-[12px] overflow-y-scroll max-h-[200px]">
                                            {["Administrator", "Member"].map((role) => (
                                                <label key={role} className="flex gap-[8px] items-center cursor-pointer">
                                                    <input type="checkbox" checked={selectedFilters.role.includes(role)} onChange={() => handleFilterChange("role", role)} className="w-[16px] h-[16px] rounded-[2px] focus:bg-brand-500 border-gray-100 focus:ring-brand-200" />
            ...
            {editDoorway && <DoorwayDetailsModel  onClose={closeModalEditDoorway} extra={true} />}
        </div >
    );
};
