...
const EditYourDesign: React.FC = () => {
  ...
  const [topLeftLogo, setTopLeftLogo] = React.useState<File | null>(null);
  const [topLeftLogoPreview, setTopLeftLogoPreview] = React.useState<string >('');
  ...
  const handleFileChangeLogo = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (file.size > maxSize) {
        ...
        console.log('in')
        setTopLeftLogo(file);
        setTopLeftLogoPreview(URL.createObjectURL(file));
      }
    }
  };
  const resetForm = () => {
      ...
      <div className="flex flex-col min-md:flex-row gap-[44px] min-md:gap-[75px] items-center min-md:items-start justify-center p-5">
          ...
          <div className="block">
            <PassPreview
              values={{
                ...
                topLogoImage:topLeftLogoPreview
              }}
            />
          </div>
        </div>
        <div className="w-[330px] flex flex-col gap-[42px]">
          ...
          <div className="grow flex flex-col gap-[7px]">
            ...
            <div
      className="grow flex flex-col gap-[7px]"
    >
      ...
      <div className="bg-[#F2F5F5] rounded-[5px] min-h-[55px] px-[11px] flex items-center">
        <label
          htmlFor="stripeImage"
          className={`cursor-pointer flex items-center w-full ${stripImagePreview?'text-[black]':'text-[#BEBEBE]'}`}
        >
              <input
        type="file"
        ...
        id="stripeImage"
        hidden
        onChange={handleFileChange} // Correct invocation here
      />
          {stripImagePreview ? (
            ...
            <div className="grow flex flex-col gap-[7px]" >
              ...
              <div className="bg-[#F2F5F5] rounded-[5px] min-h-[55px] px-[11px] flex items-center">
                ...
                <label
                  ...
                  className={`cursor-pointer flex items-center w-full ${topLeftLogoPreview?'text-[black]':'text-[#BEBEBE]'}`}
                >
                              <input
                  type="file"
                  accept="image/png, image/jpg , image/jpeg , image/JPG"
            ...
            <button
              ...
              disabled={stripImage===null && topLeftLogo===null && backgroundColor!==''}
              className={`${(stripImage===null && topLeftLogo===null && backgroundColor!=='')?'bg-[#BEBEBE]':'bg-themeColor'} text-white text-[15px] font-[500] rounded-[5px] py-[12px]`}
            >
              Submit
            </button>
          </form>
