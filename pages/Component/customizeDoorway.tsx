...
const CustomizeYourDesign: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    deviceType: state.user.passType||"",
    color: "#22242C",
 ...
 const Remove=()=>(<svg
  ...
  const handleChange = (
    ...
    setFormData((prevState) => ({
      ...prevState,
      [name]: value, // Update the field with the new string value
    }));
  };
  ...
  const handleAddField = (field: keyof typeof formData) => {
    if (Array.isArray(formData[field])) {
      setFormData((prevState) => ({
        ...prevState,
        [field]: [...prevState[field], ""], // Add an empty field (string)
      }));
    }
  };
  ...
  const handleChangeArray = (
    ...
    updatedValues[index] = value; // Update the specific index of the array field
  
    setFormData((prevState) => ({
      ...prevState,
      [field]: updatedValues, // Update the array field with the new value
    }));
  };
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newErrors = Object.keys(formData).reduce((errors, key) => {
      ...
      // Check if the value is a string and then call trim, otherwise skip
      errors[key as keyof typeof formErrors] = 
        typeof value === 'string' ? value.trim() === "" : false; // Only trim if it's a string
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="flex flex-col min-md:flex-row gap-[44px] min-md:gap-[75px] items-center min-md:items-start justify-center p-5">
        ...
        <form onSubmit={handleSubmit} className="w-[330px] flex flex-col gap-[20px]">
      {/* Device Type */}
      <div className="flex flex-col gap-[25px]">
        ...
        <div className="flex pt-[8px] gap-4 justify-between">
          ...
          >
            ...
            <input
              ...
              type="radio"
              value="apple"
              checked={formData.deviceType === "apple"}
              onChange={handleChange}
              onClick={() => {
                setFormData((prevState) => ({
                    ...prevState,
                    deviceType: enums.PASS_VIEW.APPLE, // Update the field with the new string value
                  }));
                dispatch(
                    saveCurrentUser({
            ...
            <input
              ...
              checked={formData.deviceType === "android"}
                onClick={() => {
                    setFormData((prevState) => ({
                        ...prevState,
                        deviceType: enums.PASS_VIEW.ANDROID, // Update the field with the new string value
                      }));
                dispatch(
                    saveCurrentUser({
        ...
        {formErrors.deviceType && <p className="text-red-500">Device type is required.</p>}
      </div>
      ...
      <div className="grow flex flex-col gap-[7px]">
            ...
            <input
              ...
              style={{
                ...
                opacity: 0,
                pointerEvents: "none",
              }} // Move it offscreen and make it invisible
            />        
            {formErrors.color && <p className="text-red-500">Color is required.</p>}
      ...
      <div className="grow flex flex-col gap-[7px]">
        ...
        <input
          ...
          className="bg-[#F2F5F5] focus rounded-[5px] min-h-[45px] px-[11px] text-[16px] placeholder-gray-300 outline-none w-full"
        />
        {formErrors.organization && <p className="text-red-500">Organisation is required.</p>}
      </div>
      ...
      <div className="grow flex flex-col gap-[7px]">
        ...
        <input
          ...
          className="bg-[#F2F5F5] focus   rounded-[5px] min-h-[45px] px-[11px] text-[16px] placeholder-gray-300 outline-none w-full"
        />
        {formErrors.jobTitle && <p className="text-red-500">Job title is required.</p>}
      </div>
      <div className="grow flex flex-col gap-[7px]">
  <div className="text-[#304861] text-[15px] md:text-[13px] font-[500]">
    Phone Numbers
    ...
    <Remove />
      ...
      <div className="grow flex flex-col gap-[7px]">
        ...
        <input
          ...
          className="bg-[#F2F5F5] focus rounded-[5px] min-h-[45px] px-[11px] text-[16px] placeholder-gray-300 outline-none w-full"
        />
        {formErrors.phoneNumber && <p className="text-red-500">Phone number is required.</p>}
      </div>
      {/* Submit Button */}
      <button
        type="submit"
