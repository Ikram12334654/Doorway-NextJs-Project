...
const DoorwayDetailsModel: React.FC<CloseModelProps> = ({ onClose }) => {
    ...
    const [phoneNumbers, setPhoneNumbers] = useState<string[]>(["03349388918", "03091504735"]);
    ...
    const handleSave = () => {
        if (selectedOption === "Custom" && customValue) {
            console.log("Saved custom value:", customValue);
        }
        // Handle saving logic here
    };
    const handleAddField = (setState: React.Dispatch<React.SetStateAction<string[]>>) => {
        setState((prev) => [...prev, ""]);
    };
    ...
    const renderUrlFields = (
        ...
        <div className="flex flex-col gap-[12px] items-start">
                ...
                <div key={index} className="w-full flex flex-col gap-[4px]">
                            ...
                            >
                                <div className="flex items-center flex-grow gap-[8px]">
                                    <div className="flex flex-col gap-[4px]">
                                        <span
                                            className={`font-medium whitespace-nowrap ${selectedOption ? "text-petite" : "text-gray-300"
                                                }`}
                                        >
                                            {selectedOption ? selectedOption : "Select Type"}
                                        </span>
                                    </div>
                                </div>
                                {selectedOption ? <svg
                    ...
                    <div className="flex items-center justify-stretch gap-[10px]">
                        <div className="flex-grow">
                            ...
                            >
                                <input
                                    className="border-box bg-transparent text-petite border-0 ring-transparent outline-none focus:outline-none w-full px-[12px] py-[9px] caret-brand placeholder-gray-300"
                                    type={type}
                                    value={value}
                                    onChange={(e) =>
                                        index > 0 && handleFieldChange(index, e.target.value, setState)
                                    }
                                    placeholder={placeholder}
                                    readOnly={index === 0} // Make the first field read-only
                                />
                            </div>
                        </div>
                    </div>
            ...
            >
                ...
                <span className="whitespace-nowrap">Add New {label}</span>
            </div>
        </div>
    );
    const renderFields = (
            ...
            {state.map((value, index) => (
                <div key={index} className="w-full flex flex-col gap-[4px]">
                    ...
                    <div className="flex items-center justify-stretch gap-[10px]">
                        <div className="flex-grow">
                            <div
                                className={`border-[1px] rounded-[6px] overflow-hidden text-gray-950 border-gray-100 focus-within:border-themeColor bg-white`}
